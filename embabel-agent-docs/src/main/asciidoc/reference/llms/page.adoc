...
The flat `llms` map above names one model per role, and a model belongs to one provider. That is the
right shape when a deployment will only ever be keyed for one provider. When it will not - a
bring-your-own-key application, or one that fails over between providers - give the role a provider
dimension with `roles`:
...
----
<1> The model this role means when OpenAI is the active provider
<2> Tuning configured against the role, applied whenever this role resolves to this provider

Call sites do not change: `ai.withLlmByRole("cheapest")` resolves against whichever provider is
active for the call. With no user key in play, that is the provider supplying the `default-llm`, so
a single-provider deployment needs nothing further.

Anything the caller sets explicitly still wins over what the role configures, so
`ai.withLlmByRole("cheapest").withTemperature(0.9)` keeps `0.9`.

A role that cannot be satisfied - because the active provider has no entry for it, or the model it
names is not registered - throws `NoSuitableModelException` at the point of use. It does not quietly
resolve to something else: falling back to the `default-llm` would let a role like `cheapest` become
the most capable, and most expensive, model in the deployment without anyone noticing.

Startup is a separate question, and the answer depends on whether the deployment holds a key. A
deployment that has one is expected to name models it has: an unresolvable name there is a typo,
and letting it boot would move the failure to whichever unrelated call first asks for that role.
A deployment still waiting for a key has no models registered yet by definition, so the same name
is expected and only worth reporting.

[cols="2,1,3", options="header", frame="all", grid="all"]
|===
|Configuration |At startup |Notes

|Keyed deployment, `embabel.models.llms` names an unregistered model
|Fails
|A typo, and fatal. The message names the role and the models that were available.

|Keyed deployment, `embabel.models.roles` names an unregistered model *under the deployment's own provider*
|Fails
|Same rule as the flat map. Applying it to only one of the two shapes was the original defect here.

|Keyed deployment, `embabel.models.roles` names a model under *any other* provider
|Not checked
|The point of the nested shape. That entry applies when a user brings a key for that provider, so its model is not expected to be registered here - warning about it would train people to ignore the warning.
...
|n/a
|Resolved per call against that provider's entry only. It never falls back to the flat `embabel.models.llms` map - a user's key must not silently select a model the deployment pays for.
|===

Whichever way startup goes, asking for a role that cannot be satisfied still throws at the point of
use, and the message names the role, the active provider, and the models that were available.

===== The deployment default as a role

`default-llm` may name a role rather than a model:

[source,yaml]
----
embabel:
  models:
    default-llm: cheapest       # <1>
    roles:
      cheapest:
        openai:
          model: gpt-4.1-nano
        anthropic:
          model: claude-haiku-4-5
----
<1> A role from `roles` or from the flat `llms` map, not a model name

A model name is resolved once, against the services registered at startup. A role is resolved per
call, through the same resolver chain as any other role - so the default follows whichever provider
key is active, and a deployment whose first key arrives after startup gets a working default without
a restart. That is the case this exists for: under BYOK nothing is registered before the first key,
so a default fixed at construction can only be the placeholder until the process restarts.

A model name that nothing registers falls back to the role chain as well, so `default-llm` keeps
working whichever way it was meant. Nothing existing changes: a name that does resolve against a
registered service is still resolved once and never sent through the resolvers.

A default that nothing can satisfy does not throw the way an unsatisfied role does. It falls back to
whatever `default-llm` resolved to at startup - the placeholder, in a deployment awaiting a key -
so the caller sees the same actionable "no LLM configured" error. Startup is stricter: with no
placeholder to fall back to, a `default-llm` that names nothing this deployment has registered -
whether a typo or a role whose every model is missing - is still fatal, and the message says which
of the two it is.

===== Resolving roles in code

To decide what a role means per user rather than per deployment, register a `RoleResolver` bean.
Resolvers are consulted in `Ordered` order, ahead of the configuration above, and returning `null`
delegates to the next one.
...
`RoleResolution.Credential` hands back a key and lets the platform do the rest: it looks up what the
role means under that provider, builds the service for it, and caches it per (provider, key, model)
so it is not rebuilt on every call. Return `RoleResolution.Options` to name a model and its tuning
directly, or `RoleResolution.Service` to supply an `LlmService` you built yourself.

`embabel-agent-starter-byok` already knows where to send a key for every provider BYOK supports —
Anthropic, OpenAI, DeepSeek, Mistral, Gemini and Atlas Cloud. The resolver above is therefore all
the code a deployment using any of them needs.
...
The platform carries the context across the threads it starts itself — a background agent run,
parallel actions, `OperationContext.parallelMap` — so setting it once at the request boundary is
enough. It does not reach threads your own code spawns: capture it there with
`ModelSelectionContextHolder.get()` and re-establish it inside the new thread.

===== Adding a provider of your own

For a provider the starter does not cover — a gateway, a proxy, a self-hosted endpoint — register a
`CredentialEndpointResolver`. It says where a key goes and what the service built from it should
call itself; the platform builds the client:
...
The case names the wire protocol, which is what decides the client: `OpenAiCompatible` for the
OpenAI protocol — OpenAI itself, DeepSeek, Mistral, Gemini, Atlas Cloud and most gateways — or
`Anthropic` for Anthropic's. Both carry `pricingModel`, which defaults to
`PricingModel.ALL_YOU_CAN_EAT` because a BYOK call is billed to the user's key rather than to the
deployment, and `knowledgeCutoffDate`, which is null unless you know it for the model the role named.

Your resolvers are asked first, in `Ordered` order, and the endpoints the starter already knows are
the fallback — so answering for a provider the framework handles overrides it, which is how you
route OpenAI through your own proxy. No `@Order` is needed for that; it decides only which of your
own resolvers is asked first. Return `null` for a provider you do not handle: a resolver that
answers for everything would point someone else's key at your endpoint. If nothing handles the
provider, the role fails with `NoSuitableModelException` and a log line naming it.

A resolver returns a value, so it needs no Spring context to test and nothing from
`com.embabel.agent.spi`, which <<reference.api-evolution,application code should not depend on>>.

For a wire protocol this framework has no client for, register a `CredentialLlmServiceFactory`
instead and build the service yourself. That one does name `LlmService`, an SPI type, which is why
it is the second choice rather than the first. The shipped implementations — one per protocol —
stand aside for a bean of the same name, but replacing one that way also replaces the code that
builds what your `CredentialEndpointResolver` beans resolve for that protocol.

===== Reading what a role is configured to mean

Resolution answers "what will this call use", which depends on the active key and on any
`RoleResolver` beans. A settings screen or diagnostic endpoint wants a different question — "what
does this deployment declare this role to be" — and answering it by reading `embabel.models`
directly means reimplementing the precedence between the flat and nested shapes, which is how a
settings screen ends up disagreeing with what actually runs.

`ModelProvider.configuredOptionsForRole` answers that question against the deployment's own
provider, ignoring user keys and application resolvers:

[source,kotlin]
----
val declared: String? = modelProvider.configuredOptionsForRole("cheapest")?.modelName
----

Use `resolveLlmOptions` instead when you need to know what a specific call will really do.
...
[[reference.llms.embedding-roles]]
===== Embedding roles under bring-your-own-key

`default-embedding-model` may name a **role** rather than a model.
A model name is resolved once, against the services registered at startup; a role is resolved on every call, through the same resolver chain roles use for chat.

That difference matters only for a deployment whose provider key arrives after boot.
Such a deployment registers no embedding service at startup, so a default fixed at construction can only be the placeholder — permanently, until the process restarts.
Naming a role instead means the key that satisfies every other role satisfies the embedding default too, with no restart.

[source,yaml]
----
embabel:
  models:
    default-embedding-model: documents   # <1>
    embedding-services:
      documents: text-embedding-3-small  # <2>
----
<1> A role, because `documents` is a key in `embedding-services`. Resolved per call.
<2> What the role means for a single-provider deployment.

Where the provider is not known until a key arrives, `embedding-roles` gives a role a model per provider — the embedding counterpart of `embabel.models.roles`:

[source,yaml]
----
embabel:
  models:
    default-embedding-model: documents
    embedding-roles:
      documents:
...
It is flatter than `roles`, which maps to a whole `LlmOptions`.
An embedding model takes text and returns a vector: there is no temperature to set, and the one parameter that does vary — the width — is the model's to state and the index's to follow.

Entries naming a model this deployment has not registered are reported at startup and tolerated, because that is what the shape is for: the model is built from a key supplied at runtime.

Which column is read comes from the active key.
Where there is no active key, it comes from the registered embedding services — but only where they agree on one provider.
A deployment with two registered providers and no key reads `embedding-services` instead of picking a column, because the alternative would be a choice made by bean registration order.

To resolve an embedding role for a user's own key, register an `EmbeddingRoleResolver` returning `EmbeddingRoleResolution.Credential`, and the platform builds the service through a `CredentialEmbeddingServiceFactory` and caches it per provider, key and model.
`embabel-agent-starter-byok` ships one covering every OpenAI-compatible provider, so this usually needs no application code.

[NOTE]
====
Embedding roles resolve through their own chain, separate from `RoleResolver`.
A chat role and an embedding role may share a name, and a resolver that answers for one must not be asked about the other.
An application that resolves both from one source registers two beans over one shared lookup — `resolve` takes the same arguments in both interfaces and returns a different type, so one Kotlin class cannot implement both.
====

[NOTE]
====
Asking for an embedding role that nothing satisfies throws, whether or not the deployment is awaiting a key — unlike a chat role, which falls back to the placeholder.
A vector index is created at a model's width, so nothing can stand in for one.
The *default* keeps its placeholder: a caller asking for whatever the deployment has is entitled to a service that says it has nothing.
====


[[reference.llms.local-models-after-startup]]
===== Locally served models that appear after startup

A local runner — Docker Model Runner, LM Studio, Ollama — is enumerated at startup, and each model it was serving is registered as a bean.
A model pulled a minute later is invisible to that, however reachable it is, because the only thing that would have seen it ran once while the platform was being built.

So a role may also be satisfied by asking the runner what it is serving *now*.
This is the local counterpart of the credential path, seam for seam: configuration names the model per provider, the resolver is asked per call, and the service is built on the spot.
`docker model pull` followed by a request is enough; the model needs no restart to become usable, and nothing distinguishes it from one that was already there.

Name the model under the **provider column**, not in the flat `llms` / `embedding-services` map:
    ...
    roles:
      cheapest:
        docker:
          ...
          temperature: 0.0              # <3>
----
<1> The embedding model this role means on a Docker runner. Need not be pulled yet.
<2> The same for a chat role. The provider key is `docker`, `lmstudio` or `ollama`.
<3> Options configured beside the model travel with it, exactly as for a registered model.

The flat maps are validated at startup and a name nothing has registered is fatal there — correctly, since in a deployment that holds its models a missing name is a typo.
A provider column is not, because its entries name models this process may have no way to serve yet: `embedding-roles` is never checked, and a `roles` entry under a runner's column is reported and tolerated rather than fatal, so an appliance whose default LLM is local still starts before anything has been pulled.
A runner declares that through `LateArrivingModels`, implemented by its `LocalModelCatalog`; without it — or with discovery disabled, since then nothing will ever ask the runner — the same entry under the deployment's own provider is still a typo and still fatal.

The runner is asked at most once per refresh interval, so this is not an HTTP round trip per embedding:
  ...
  agent:
    platform:
      models:
        local-discovery:
          enabled: true                 # <1>
          refresh-interval: 10s         # <2>
          miss-refresh-interval: 1s     # <3>
----
<1> Off restores the startup-only behaviour: a model pulled after boot stays invisible until the process comes back.
<2> How long a list of served models is reused. Also the floor on how long a model that has just been *removed* keeps resolving.
<3> How often a model absent from the cached list may force a fresh look, so a model pulled two seconds ago is usable on the next request rather than the one after the refresh interval.

A role is declined, and left to the rest of the chain, in three cases: configuration says nothing about this provider; the runner is not serving the named model, so an absent runner simply answers nothing; or the call carries a user key for a *different* provider, since a user who brought a key asked for their provider and a model on this machine would answer with the wrong one.

A locally served model is also **listed and usable by name**, not only reachable through a role.
`listModelNames` and `listModels` report what the runners are serving now alongside what was registered at startup, so a model pulled a moment ago appears in the UI meant to select it; asking for one of those names — `getLlm`, `getEmbeddingService`, `default-llm`, `default-embedding-model` — resolves it.
A model registered at startup wins over a runner serving the same name, and appears once.

A by-name lookup has a name to miss on, so it re-asks the runner and finds a just-pulled model within `miss-refresh-interval`; a listing has nothing to miss on and is served from the snapshot, so it can trail a by-name lookup by up to `refresh-interval`.
Closing that gap would mean an HTTP round trip per listing, which is the cost the snapshot exists to avoid.

`default-llm` and `default-embedding-model` are asked of the runners only when they name a *model*.
When either names a role it resolves through the chain, which already contains the local resolvers — so asking again by that name could only miss, and a miss is a request.

Which of the two lists a model appears in comes from the runner: LM Studio reports a type per model, while Docker and Ollama read the same configuration their startup registration reads, so a model cannot change category by virtue of having arrived late.
That classification decides what a caller naming a model gets back — a chat model will not answer an embedding request by name — but it does not gate role resolution, where the role that asked is the better signal.

Late models are described by name and provider only.
The rest of a model's metadata is read off a built service, and building one per entry would turn a listing into a round of work against the runner.

To do the same for a runner the framework does not ship, implement `LocalModelSource` — what it is serving, with what each model is for, and how to build a service for one — and publish the three beans `LocalModelBeans` builds over it: the catalog, and the two resolvers sharing it.
