...
When streaming structured objects, `createObjectStreamWithThinking` expects the target class to produce a JSON object schema (`{...}`).
Passing `String.class` directly causes the LLM to emit bare JSON strings such as `"July - 29°C"` instead of JSON objects,
which the streaming parser cannot recognize as structured output events — they are misclassified as thinking content.

Use `StringResult` from `com.embabel.common.ai.converters.streaming` as a wrapper:
...
`StringResult` produces schema `{"type":"object","properties":{"value":{"type":"string"}}}`, ensuring the LLM returns
`{"value":"..."}` which is correctly parsed and emitted as an `isObject()` event.
