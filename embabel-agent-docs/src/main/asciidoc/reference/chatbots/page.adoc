...
Durable tracking is explicitly opt-in. The application must construct or inject an `AssetStore` and pass it when wrapping the tool; no no-op store is installed automatically.
